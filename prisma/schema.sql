-- Idempotent Type Creation using DO blocks
DO $$ BEGIN
...
DO $$ BEGIN
    CREATE TYPE "RSVPStatus" AS ENUM ('ACCEPTED', 'MAYBE', 'DECLINED', 'PENDING', 'WAITLISTED', 'CANCELLED');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN
    CREATE TYPE "BroadcastAudience" AS ENUM ('ALL', 'CONFIRMED', 'PENDING');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
-- CreateIndex IF NOT EXISTS (Postgres handles this via separate statements or just ignoring)
...
-- AddForeignKey IF NOT EXISTS (via DO blocks)
