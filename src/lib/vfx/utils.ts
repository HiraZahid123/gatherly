/**
 * Lightweight Perlin/Simplex-style Noise Utility
 * Used for smooth, deterministic organic motion.
 */
...
export const noise2D = (x: number, y: number) => {
    ...
    return lerp(v, lerp(u, grad(PERM[AA], x, y, 0),
...
/**
 * Normalizes noise value to 0-1 range
 */
export const perlin2D = (x: number, y: number) => (noise2D(x, y) + 1) / 2;
